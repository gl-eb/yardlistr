yardlistr <- function(location, dir_dat, dir_img) {
  ...
  dir_dat <- dir_dat |> as_fs_path()
  dir_img <- dir_img |> as_fs_path()

  # get all ebird files
  files <- dir_ls(dir_dat)
  ...
  yardcount <- tibble(
    ...
    datetime = floor_date(checklists$datetime, unit = "day"),
  ...
  for (i in seq_len(dim(checklists)[1])) {
    ...
    current_species <- dat |>
      filter(datetime == deframe(checklists[i, "datetime"])) |>
  ...
  # list of time units
  # timeunits <- tibble(week = seq_len(53), n = rep(NA, 53))
  n_timeunits <- 48
  timeunits <- tibble(
  ...
  checklists_per_timeunit <- dat |>
    ...
    count() |>
    ungroup()
  # stats_checklists <- checklists_per_timeunit |>
  #   summarise(
  #     total = sum(n),
  #     max = max(n)
  #   )
  checklists_per_timeunit <- checklists_per_timeunit |>
    ...
    pivot_wider(names_from = month_tetrad, values_from = frequency) |>
    add_column(
  ...
  for (i in seq_along(timeunits$month_tetrad)) {
    ...
    dat_timeunit <- dat |>
      filter(month_tetrad == deframe(timeunits[i, "month_tetrad"]))
  ...
  frequency_per_timeunit <- checklists_per_timeunit |>
    ...
    arrange(taxon) |>
    mutate(
      species = species |> as_factor() |> fct_rev(),
  ...
  frequency_year <- dat |>
    ...
    mutate(
      species = species |> as_factor() |> fct_rev(),
      frequency = n / complete |> select(datetime) |> deframe() |> length(),
      frequency
    )
  ...
  location_short <- str_split(location, boundary("word"))[[1]][1]
  ...
  plot_heatmap <- frequency_per_timeunit |>
    mutate(month_label = month(month, label = TRUE) |> as_factor()) |>
